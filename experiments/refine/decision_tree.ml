...
let choose_branch_var (rows : row list) =
  ...
  in
  print_endline (show_counts final_counts);
  find_max_count (None, final_counts)

type ctor_case = [ `CtorCase of ctor * var list * row list ]
...
let rec compile_ctor_cases ctx (rows : row list) (branch_var : var)
  ...
  let extend_subrows_of_case row (`CtorCase (ctor, vars, rows)) =
    `CtorCase (ctor, vars, row :: rows)
  in

  let go_row cases row =
    print_endline (show_row row);
    let row, col = remove_col branch_var row in
    match col with
    | Some (_, pat) -> (
        match pat with
        | Wild -> (* nothing more to test here, we pass through *) cases
        | Ctor (ctor, arg_pats) ->
            ...
            let new_ctor_case =
              ...
              let new_sub_rows = subrow :: sub_rows in
...
and compile_rows ctx = function
  | [] -> failwith "empty rows!"
  | (cols, body) :: _ when cols = [] ->
      (* A branch with no columns always matches, since there is nothing more
         ...
         are redundant; for now, we drop them. *)
      Immediate body
  | rows -> (
      print_endline (show_rows rows);
